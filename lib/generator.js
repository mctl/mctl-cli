 ...
 * @param {*} name
 */
 function getCamelName(name, isUpper) {
  const strArr = name.split('-');

  return strArr.reduce((total, item, index) => {
    if (index === 0) {
      if (isUpper) {
        return total + '' + getUpper(item);
      } else {
        return total + '' + item;
      }
    } else {
...
class Generator {
  ...
  async create(url, name, tips) {
    ...
    if (result) {
      ...
      const files = glob.sync(`${name}/**/*`, { nodir: true, dot: true });

      files?.forEach(async (file) => {
        const filePath = path.resolve(process.cwd(), file);
        const data = {
          name,
          lowerCamelName: getCamelName(name),
          upperCamelName: getCamelName(name, true),
        };
